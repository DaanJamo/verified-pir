...
From MetaCoq.Common Require Import BasicAst Kernames config.
...
Definition print_name (na : name) : string :=
  ...
  Program Definition print_box_type (Σ : P.global_env_ext) (wfextΣ : ∥ PCUICTyping.wf_ext Σ∥) (tvars : list name) :=
  fix f (bt : box_type) :=
    match bt with
    | TBox => "□"
    | TAny => "𝕋"
  ...
  Definition print_box := print_box_type Σ (todo "assume wf_env") [].
  ...
  Fixpoint print_term_annotated
    ...
    match t with
    | tBox => fun bt => "□ : " ^ "TBox"
    | tRel i => fun bt => print_name (nth i Γ nAnon) ^ " : " ^ print_box bt
    | tLambda na body =>
      fun '(bt, a) =>
