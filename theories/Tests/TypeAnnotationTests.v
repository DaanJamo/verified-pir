From MetaCoq.Erasure.Typed Require Import Annotations Erasure TypeAnnotations.
From MetaCoq.Erasure.Typed Require Import ExAst.
From MetaCoq.PCUIC Require Import PCUICAst.
From MetaCoq.TemplatePCUIC Require Import TemplateToPCUIC.
From MetaCoq.PCUIC Require Import PCUICTyping PCUICSN.
...
Check fun e => flag_of_type_impl (empty_ext (PCUICProgram.trans_env_env e)) _ _ _ _ _ _ _ _.
...
Program Definition erase_type_program (p : Ast.Env.program) : global_env_ext * (list name * box_type) :=
  let Σ := trans_global_env p.1 in
  let Σext := empty_ext (PCUICProgram.trans_env_env Σ) in
  (Σext, erase_type_impl Σext _ extraction_normalizing _ Σext eq_refl (trans Σ p.2) _).
Next Obligation. Admitted.
Next Obligation. Admitted.
Next Obligation. Admitted.
...
Section printing.
  ...
  Fixpoint print_term_annotated
           (Γ : list name)
           (t : E.term) : annots box_type t -> string :=
...
Import Extraction ResultMonad Optimize Utils.
...
Definition translate_program (p : T.Env.program) (opt : bool) (ignore : list kername) (TT : list (kername * string)) 
  : result (∑ et : E.term, annots box_type et) string.
Proof.
  refine (let entry := match p.2 with
           | T.tConst kn _ => kn
           | T.tInd ind _ => (inductive_mind ind)
  ...
  { destruct opt; cbn.
    - constructor; [|constructor].
      apply annot_dearg_transform.
    - constructor. }
  destruct extract_template_env as [|e]; [|exact (Err "could not translate environment")].
  destruct (bigprod_find (fun '(kn, _, _) _ => eq_kername entry kn) X); [|exact (Err "could not find constant in environment")].
  destruct s as ((? & decl) & annot).
  destruct decl; [|exact (Err "expected term to be a constant, not an inductive")|exact (Err "expected term to be a constant")].
  cbn in *.
  unfold constant_body_annots in *.
  destruct Ex.cst_body; [|exact (Err "cst_body empty")].
  exact (Ok (t0; annot)).
Defined.

Definition extract_and_print_opt p := general_extract_typed p true [] [].
Definition extract_and_print_no_opt p := general_extract_typed p false [] [].
...
Module ex1.
  ...
  Example test :
    extract_and_print_opt ex = "(S : nat → nat) (O : nat) : nat".
  Proof. vm_compute. reflexivity. Qed.
End ex1.
